...
class DenoiserInput:
    ...
    def format_input(
        ...
        if not (np.allclose(kperp, csts.kperp) and np.allclose(kpar, csts.kpar)):
            raise NotImplementedError('Interpolation of input kperp and kpar bins is not supported yet.')

        if len(ps_realisations.shape) == 2:
            ps_realisations = ps_realisations[np.newaxis,...]
        if ps_realisations.shape[1] != len(kperp):
            raise ValueError('You supplied the wrong kperp bins: %s kperp bins in PS vs %s kperp bins in kperp array supplied.' % (ps_realisations.shape[1], len(kperp)))
        if ps_realisations.shape[2] != len(kpar):
            raise ValueError('You supplied the wrong kpar bins: %s kpar bins in PS vs %s kpar bins in kpar array supplied.' % (ps_realisations.shape[2], len(kpar)))
        if len(ps_realisations.shape) > 3:
            raise ValueError('The shape of the input ps_realisations PS should be (Nsamples, Nkperp, Nkpar).')
        ...
        if not np.allclose(kperp, csts.kperp):
            warnings.warn('Input kperp bins are not the same as the expected kperp bins! Proceeding anyways...')
        if not np.allclose(kpar, csts.kpar):
            warnings.warn('Input kpar bins are not the same as the expected kpar bins! Proceeding anyways...')
        if ps_realisations.unit != un.mK**2:
            raise ValueError('Input PS should be in mK^2, but is in %s.' % ps_realisations.unit)
        if kperp.unit != un.Mpc**(-1):
            raise ValueError('Input kperp should be in Mpc^{-1}, but is in %s.' % kperp.unit)
        if kpar.unit != un.Mpc**(-1):
            raise ValueError('Input kpar should be in Mpc^{-1}, but is in %s.' % kpar.unit)
    ...
    def normalize_noisy(self, noisy: np.ndarray) -> np.ndarray:
        ...
        return transform(torch.Tensor(noisy), torch.Tensor(csts.noisy_scale), torch.Tensor(csts.noisy_bias)).cpu().detach().numpy()
